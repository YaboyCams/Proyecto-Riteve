            ...
            if elegida.get() == 1: # Meter árbol acá
                if mes.get() and dia.get() and hora.get() and mins.get():
                    ...
                    if fecha_formato not in citas_disponibles:
                        ...
                        return
                    datos_cita = [cita.cget(), tipo_revision.get(), listbox_vehiculo.curselection(), num_placa.get(), marca_v.get(), modelo_v.get(), usuario.get(), telefono_u.get(), correo_u.get(), direccion_u.get(), fecha_formato, "PENDIENTE"]
                    boton_guardar_cita.config(state = NORMAL)
            elif listbox.curselection(): 
                datos_cita = [cita.cget(), tipo_revision.get(), listbox_vehiculo.curselection(), num_placa.get(), marca_v.get(), modelo_v.get(), usuario.get(), telefono_u.get(), correo_u.get(), direccion_u.get(), listbox.curselection(), "PENDIENTE"]
                boton_guardar_cita.config(state = NORMAL)
        else:
